classdef op_obj_image < op_obj
    ...
    properties (Access = protected)
        ...
        d_type = 'data_obj_image_vector'; % applies to this data type

    end

    methods

        function O = op_obj_image(S, h_rhs, h_lhs, k)
        ...
        function data_rhs = init_x(O, a, b)
            if (nargin < 2), a = O.n_vox; end
            if (nargin < 3), b = O.n_mp; end
            h = O.h_rhs;
            h.dim(5) = O.n_mp;
            data_rhs = data_obj_image_vector(zeros(a,b), h);
        end

        function data_lhs = apply(O, data_rhs)
            assert(my_isa(data_rhs, O.d_type), 'rhs not a %s', O.d_type);
            data_lhs = data_obj_image_vector(O.aM * data_rhs.w(:, O.k), O.h_lhs);
        end

        function data_rhs = apply_adjoint(O, data_lhs)
            assert(my_isa(data_lhs, O.d_type), 'rhs not a %s', O.d_type);
            data_rhs = data_obj_image_vector(O.aMT * data_lhs.w(:, O.k), O.h_rhs);
        end
